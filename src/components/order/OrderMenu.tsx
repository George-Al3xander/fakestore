...
const OrderMenu = () => {
    ...
    return(<div className="border-[1px] w-[100%] md:basis-[40%] rounded-xl p-4 self-start">
        ...
        <NavLink to={step ? "/order/complete" :"/order/checkout"}>
            <button disabled={step ? !orderInfoValid : false}  className="whitespace-nowrap text-accent bg-primary-500 px-5 py-3  rounded-full  w-[100%] disabled:opacity-50 disabled:cursor-not-allowed">{step ? "Place an order" : "Checkout"} | ${total.toFixed(2)}</button>
        </NavLink>
