...
const Footer = () => {
    ...
    return(<footer className="text-accent py-10">
        <div className="w-responsive m-auto flex flex-col md:flex-row gap-10">
            <div key="footer-div-1" className="basis-[50%]">
            ...
            <div key="footer-div-2" className="basis-[40%]">
                <h1 className="uppercase  text-lg font-bold mb-4">Quick link</h1>
                <nav className="flex flex-col gap-2">
                    <NavLink key="footer-link-1" className={"capitalize opacity-70 hover:opacity-100 hover:text-primary-500 transition-all duration-300"} to={"/"}>Home</NavLink>
                    <NavLink key="footer-link-2" className={"capitalize opacity-70 hover:opacity-100 hover:text-primary-500 transition-all duration-300"} to={"/shop"}>Shop all</NavLink>
                    {Array.isArray(categories) &&  categories.length > 0 ? categories.map((cat, index) => {
                        return <NavLink key={"footer-link-" + index + 1} className={"capitalize opacity-70 hover:opacity-100 hover:text-primary-500 transition-all duration-300"} to={`${cat}`}>{cat}</NavLink>
                    }): null}
                </nav>
            </div>
            <div key="footer-div-3" className="basis-[20%]">
